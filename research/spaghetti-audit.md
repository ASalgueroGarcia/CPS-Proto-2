...
## 26. Phase 5 execution log — structural chunk (2026-09-23 night, branch `feature/phase5-structural`)

Dima's call: config calls (shadow atlas, asmdef, CI) noted and deferred to a team call; Phase 5 proceeds. Branch stacks on `feature/cleanup` (PR #40). Four chunks, each its own commit:

| Commit | Chunk |
|---|---|
| `cbf083a` | **AOE helper** — new `Scripts/Core/AOEDamage.cs` (`Burst(origin, radius, playerDamage, enemyDamage, knockbackForce, exclude)`): resolves `Health` via the collider or any parent, classifies by the Health's ROOT tag, routes damage + knockback through `Health.TakeDamage`, dedupes per-victim. `PlayerFSM.ExecuteSpecialAttackDamage` (player slot = 0 → enemies only) and `Projectile.Explode` (full player / half enemy) now call it. Rulings: (a) per-victim DEDUPE is a real fix — multi-collider enemies previously took the burst once per collider; (b) victim classification is now root-tag-based — projectile splash previously MISSED enemy child colliders that were untagged; (c) `ExplodingTrap` deliberately keeps its bespoke burst (lifted direction + direct rigidbody forces = trap physics, not the same pattern) — the audit's "5 implementations" anarchy is now 2 → 1 shared + 1 bespoke by design; (d) dead `PlayerFSM.enemyLayer` field removed (was only written at init, never read) — the Player prefab drops the serialized value on reimport. |
| `4ae94f3` | **Event-driven HUD** — `UIManager.UpdatePlayerUI` (rebuilt speed/damage/combo/coins/inventory strings EVERY frame) replaced by events: `PlayerStatsManager.OnCoinsChanged(int)` (AddCoins/TrySpendCoins/reset) + `PlayerStatsManager.PlayerStatsApplied` (power-up apply, BindToPlayer re-apply, reset) + `PlayerFSM.OnComboChanged(int)` via a new `SetComboStep` (all 4 combo mutation sites route through it). `UIManager.BindToPlayer` subscribes idempotently (`-=` then `+=`) and does the initial paint; per-frame work is now ONLY the cooldown dial with a `_specialReadyShown` latch (no per-frame "READY" string writes). Health HUD was already event-driven. |
| `2131273` | **Dead stats purge** — `currentCriticalDamage`/`currentSpecialDamage`/`currentAttackSpeed` were accumulated but NEVER read (audit §5 "dead stat fields"); the CriticalDamage/SpecialDamage power-up cases become explicit design-ask placeholders (they were already gameplay no-ops), reset assignments removed, Spanish `Prints()` + its commented call sites removed. |
| `e1ac733` | **Projectile pooling (§23 step 5)** — `RangedAttack` owns one `ObjectPool<GameObject>` per strategy component (`createFunc` binds `Projectile.BindToPool`, `actionOnRelease` deactivates, `collectionCheck: true`); projectiles stay parented to the room (outlive the firing enemy, die with the room). `Projectile` gains `pooledDespawn` + `BindToPool` + `Despawn()` — all four destroy sites (player hit, ground/default, explode, 5s lifetime) route through it; `Setup()` doubles as the full reuse reset (re-caches rb, relaunches velocity, restarts the lifetime Invoke). Explosion VFX stays Instantiate/Destroy (one-shot effect, not pooled). |

**Verification (static):** msbuild clean after every chunk (same two pre-existing warnings only); brace balance + line endings verified per-file. **Playtest gate pending (Dima):** special attack hits each enemy exactly once; ranged rooms respawn enemies AND projectiles across 3+ waves (Kill All + live combat) with zero pool errors; HUD strings update on coin pickup, power-up purchase, combo steps; cooldown dial still animates; explosion knockback feels unchanged; shop purchase flow unchanged.

**Not in this branch (unchanged):** stats single-source-of-truth beyond the dead-field purge — the remaining dual-tracking (FSM publics ↔ PlayerStatsManager privates, Health ↔ SyncHealth mirror) is load-bearing run-persistence logic and stays until the meta-progression/RunManager design answer; `RunManager`, `asmdef`, CI, shadow atlas, Arena Flat tessellation — noted in §25.
